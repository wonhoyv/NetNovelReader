...
class SearchViewModel(val context: Application) : AndroidViewModel(context),
    ...
    val selectHotWordCommand = ReaderLiveData<String>()               //选中的hotword的text
     ...
     */
    override fun downloadBook(bookname: String, catalogUrl: String, chapterName: String?, which: Int) = launch {
    ...
    override fun showDialogTask(itemDetail: SearchBean) {
        showDialogCommand.value = itemDetail
    }

    override fun detailClickTask(itemText: String) {
    ...
    override fun activityExitTask() {
        exitCommand.call()
    }

    //将搜索热词填充到searchView上但是不触发网络请求
    override fun selectHotWordTask(word: String) {
        selectHotWordCommand.value = word
    }
